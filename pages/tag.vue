<template>
  <div class="mt-12 min-h-screen-md">
    <div class="flex text-xl items-center justify-center w-full">
      <OnClickOutside @trigger="isSelectionOpen = false" class="sm:w-80 lg:w-1/3 lg:px-6">
        <div class="relative flex items-center">
          ...
          <div
            ...
            "
          >
            <h5 v-if="searchSupabaseTags.length" class="px-4 pt-3 uppercase text-sm text-light-900 opacity-50">
              Supabase
            </h5>
            <router-link
              class="
            ...
            <h5 v-if="searchTags.length" class="px-4 pt-3 uppercase text-sm text-light-900 opacity-50">Tagging</h5>
            <router-link
              class="
                inline-flex
                justify-between
...
const searchSupabaseTags = computed(() => {
  ...
  return supabaseTags.value.filter((i) => i.tags.toLocaleLowerCase().indexOf(s) > -1)
})
